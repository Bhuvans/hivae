...
def loglik_real(batch_data,list_type,theta,normalization_params,tau2,kernel_initializer,name):
...
#    est_var = 0.05*tf.ones_like(est_var)
    ...
    #Compute loglik
    log_p_x = -0.5 * tf.reduce_sum(tf.squared_difference(data,est_mean)/est_var,1) - int(list_type['dim'])*0.5*tf.math.log(2*np.pi) - 0.5*tf.reduce_sum(tf.math.log(est_var),1)
...
def loglik_pos(batch_data,list_type,theta,normalization_params,tau2,kernel_initializer,name):
...
def loglik_cat(batch_data,list_type,theta,normalization_params,tau2,kernel_initializer,name):
...
def loglik_ordinal(batch_data,list_type,theta,normalization_params,tau2,kernel_initializer,name):
    output = dict()
...
def loglik_count(batch_data,list_type,theta,normalization_params,tau2,kernel_initializer,name):
