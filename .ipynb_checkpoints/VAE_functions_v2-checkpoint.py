...
def place_holder_types(types_file, batch_size):
    ...
    for i in range(len(types_list)):
        batch_data_list_observed.append(tf.compat.v1.placeholder(tf.float32, shape=(batch_size,types_list[i]['dim'])))
...
def batch_normalization(batch_data_list, types_list, miss_list):
    ...
    for i,d in enumerate(batch_data_list):
        ...
        condition_indices = tf.dynamic_partition(tf.range(tf.shape(input=d)[0]), miss_list[:,i], num_partitions=2)
...
def s_proposal_multinomial(X, batch_size, s_dim, tau, reuse):
    ...
    log_pi = tf.compat.v1.layers.dense(inputs=X, units=s_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'enc_s', reuse=reuse)
...
#                         kernel_initializer=tf.random_normal_initializer(stddev=0.05), name='layer_0_' + 'mean_enc_z', reuse=reuse)
    ...
    mean_qz = tf.compat.v1.layers.dense(inputs=tf.concat([X,samples_s],1), units=z_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'mean_enc_z', reuse=reuse)
    log_var_qz = tf.compat.v1.layers.dense(inputs=tf.concat([X,samples_s],1), units=z_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'logvar_enc_z', reuse=reuse)
...
def z_proposal_Normal(X, batch_size, z_dim, reuse):
    ...
    #We propose a GMM for z
    mean_qz = tf.compat.v1.layers.dense(inputs=X, units=z_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'mean_enc_z', reuse=reuse)
    log_var_qz = tf.compat.v1.layers.dense(inputs=X, units=z_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'logvar_enc_z', reuse=reuse)
...
def z_proposal_GMM_factorized(X, samples_s, miss_list, batch_size, z_dim, reuse):
    ...
    for i,d in enumerate(X):
        ...
        nObs = tf.shape(input=observed_data)[0]
        
        #Mean layer
        aux_m = tf.compat.v1.layers.dense(inputs=tf.concat([observed_data,observed_s],1), units=z_dim, activation=None,
                             kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'mean_enc_z'+str(i), reuse=reuse)
        ...
        #Logvar layers
        aux_lv = tf.compat.v1.layers.dense(inputs=tf.concat([observed_data,observed_s],1), units=z_dim, activation=None,
                             kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name='layer_1_' + 'logvar_enc_z'+str(i), reuse=reuse)
...
def z_distribution_GMM(samples_s, z_dim, reuse):
    ...
    mean_pz = tf.compat.v1.layers.dense(inputs=samples_s, units=z_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05), name= 'layer_1_' + 'mean_dec_z', reuse=reuse)
...
def theta_estimation_from_z(samples_z, types_list, miss_list, batch_size, reuse):
    ...
    for i,d in enumerate(types_list):
        ...
        condition_indices = tf.dynamic_partition(tf.range(tf.shape(input=samples_z)[0]), miss_list[:,i], num_partitions=2)
...
def observed_data_layer(observed_data, missing_data, condition_indices, output_dim, name, reuse, bias):
    ...
    obs_output = tf.compat.v1.layers.dense(inputs=observed_data, units=output_dim, activation=None,
                         kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05),name=name,reuse=reuse,trainable=True,use_bias=bias)
    miss_output = tf.compat.v1.layers.dense(inputs=missing_data, units=output_dim, activation=None,
                     kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=0.05),name=name,reuse=True,trainable=False,use_bias=bias)
    #Join back the data
